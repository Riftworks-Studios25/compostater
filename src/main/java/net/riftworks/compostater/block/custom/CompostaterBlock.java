package net.riftworks.compostater.block.custom;
...
public class CompostaterBlock extends Block implements InventoryProvider {
    ...
    public CompostaterBlock(AbstractBlock.Settings settings) {
        super(settings);
    ...
    public static BlockState emptyFullComposter(Entity user, BlockState state, World world, BlockPos pos) {
        if (!world.isClient) {
            ...
            ItemEntity itemEntity = new ItemEntity(world, vec3d.getX(), vec3d.getY(), vec3d.getZ(), new ItemStack(Items.POTATO));
    ...
    public SidedInventory getInventory(BlockState state, WorldAccess world, BlockPos pos) {
        int i = (Integer)state.get(LEVEL);
        if (i == 8) {
            return new CompostaterBlock.FullComposterInventory(state, world, pos, new ItemStack(Items.POTATO));
        } else {
            return (SidedInventory)(i < 7 ? new CompostaterBlock.ComposterInventory(state, world, pos) : new CompostaterBlock.DummyInventory());
        }
    }
    ...
    static class ComposterInventory extends SimpleInventory implements SidedInventory {
        ...
        public boolean canInsert(int slot, ItemStack stack, @Nullable Direction dir) {
            return !this.dirty && dir == Direction.UP && CompostaterBlock.ITEM_TO_LEVEL_INCREASE_CHANCE.containsKey(stack.getItem());
        }
        ...
        public void markDirty() {
            ...
            if (!itemStack.isEmpty()) {
                this.dirty = true;
                BlockState blockState = CompostaterBlock.addToComposter(null, this.state, this.world, this.pos, itemStack);
    ...
    static class FullComposterInventory extends SimpleInventory implements SidedInventory {
        ...
        @Override
        public boolean canExtract(int slot, ItemStack stack, Direction dir) {
            return !this.dirty && dir == Direction.DOWN && stack.isOf(Items.POTATO);
        }

        @Override
        public void markDirty() {
            CompostaterBlock.emptyComposter(null, this.state, this.world, this.pos);
            this.dirty = true;
        }
    }
}
